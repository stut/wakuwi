  ...
  return (
    <div className="flex flex-col h-full">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {showPortForward && (
              <Button
                ...
                size="sm"
                onClick={() => {
                  const firstPort = pod.containers
                    .flatMap((c) => c.ports ?? [])
                    .find(Boolean)
            ...
            <Button
              ...
              onClick={() =>
                onNavigate(
                  `/${encodeURIComponent(context)}/${encodeURIComponent(namespace)}/pods/${encodeURIComponent(name)}/logs`,
                )
              }
            >
            ...
            <Button
              ...
              onClick={() =>
                onNavigate(
                  `/${encodeURIComponent(context)}/${encodeURIComponent(namespace)}/pods/${encodeURIComponent(name)}/manifest`,
                )
              }
            >
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4 content-start overflow-auto">
        <section className="rounded-lg border bg-card shadow-sm p-4">
          ...
          <dl className="grid grid-cols-1 gap-y-2 text-sm">
            <div>
              ...
              <dd className={cn("font-medium mt-0.5", statusClass(pod.status))}>
                {pod.status}
              </dd>
            </div>
            <div>
              <dt className="text-muted-foreground text-xs">Phase</dt>
              <dd className="mt-0.5">{pod.phase}</dd>
            </div>
            <div>
            ...
            <div>
              <dt className="text-muted-foreground text-xs">IP</dt>
              <dd className="font-mono text-xs mt-0.5">{pod.ip || "—"}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground text-xs">Created</dt>
              <dd className="text-xs mt-0.5">
                {formatDate(pod.createdAt)}{" "}
                <span className="text-muted-foreground">({pod.age} ago)</span>
              </dd>
            </div>
            <div>
              <dt className="text-muted-foreground text-xs">Namespace</dt>
              <dd className="text-xs mt-0.5">{pod.namespace}</dd>
            </div>
          </dl>
        </section>
          ...
          >
            ...
            <dl className="grid grid-cols-1 gap-y-2 text-sm">
              <div>
                ...
                <dd className={cn("mt-0.5 font-medium", statusClass(c.state))}>
                  {c.state}
                </dd>
              </div>
              <div>
              ...
              <div>
                <dt className="text-muted-foreground text-xs">Restarts</dt>
                <dd className="mt-0.5">{c.restarts}</dd>
              </div>
              {c.ports && c.ports.length > 0 && (
                <div>
                  <dt className="text-muted-foreground text-xs">Ports</dt>
              ...
              <thead>
                <tr className="border-b">
                  {["Type", "Status", "Reason", "Message"].map((h) => (
                    <th
                      key={h}
                      className="pb-1.5 text-left font-medium text-muted-foreground pr-4"
                    >
                      {h}
                    </th>
                  ))}
        ...
        {(pod.owners ?? []).length > 0 &&
          (() => {
            const grouped = pod.owners!.reduce<
            ...
            return Object.entries(grouped).map(([oKind, items]) => {
              const cols = RESOURCE_COLUMNS[oKind] ?? [
                { key: "name", label: "Name" },
                { key: "age", label: "Age" },
              ]
              return (
                      ...
                      <tbody>
                        {items.map((item) => (
                          <tr
                            ...
                            onClick={() =>
                              onNavigate(
                                `/${enc(context)}/${enc(namespace)}/${enc(oKind)}/${enc(item.name)}`,
                              )
                            }
                          >
                            {cols.map((col) => (
