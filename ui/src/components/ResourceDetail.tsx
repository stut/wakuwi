...
import type { ResourceDetail as ResourceDetailType, RelatedResource } from "@/types"
...
export function ResourceDetail({ kind, context, namespace, name, onNavigate }: Props) {
  ...
  const load = useCallback(() => {
    ...
    fetchJSON<ResourceDetailType>(
      ...
      .then((data) => { setDetail(data); initialLoad.current = false; setLoading(false) })
  ...
  if (loading && !detail) return <div className="flex h-full items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></div>
      ...
      <div className="pb-4 mb-4 border-b shrink-0">
        <div className="flex items-start justify-between gap-4">
          ...
          <Button variant="outline" size="sm" className="shrink-0"
            onClick={() => onNavigate(`/${enc(context)}/${enc(namespace)}/${enc(kind)}/${enc(name)}/manifest`)}>
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4 content-start overflow-auto">
        ...
        <section className="rounded-lg border bg-card shadow-sm p-4">
          ...
          <dl className="grid grid-cols-1 gap-y-2 text-sm">
            <div><dt className="text-muted-foreground text-xs">Name</dt><dd className="font-medium mt-0.5">{detail.name}</dd></div>
            <div><dt className="text-muted-foreground text-xs">Namespace</dt><dd className="mt-0.5">{detail.namespace}</dd></div>
            <div><dt className="text-muted-foreground text-xs">Age</dt><dd className="mt-0.5">{detail.age}</dd></div>
            <div><dt className="text-muted-foreground text-xs">Created</dt><dd className="text-xs mt-0.5">{formatDate(detail.createdAt)}</dd></div>
          </dl>
        </section>

        {detail.sections.map((section) => (
          <section key={section.title} className="rounded-lg border bg-card shadow-sm p-4">
            <h2 className="text-sm font-semibold mb-3">{section.title}</h2>
            <dl className="grid grid-cols-1 gap-y-2 text-sm">
              {section.items.map((item) => (
                <div key={item.key} className="">
                  <dt className="text-muted-foreground text-xs">{item.key}</dt>
                  <dd className="font-mono text-xs mt-0.5 break-all">{item.value || "—"}</dd>
                </div>
              ))}
            </dl>
          </section>
        ...
        {(detail.related ?? []).length > 0 && (() => {
          const grouped = (detail.related!).reduce<Record<string, RelatedResource[]>>((acc, r) => {
            (acc[r.kind] ??= []).push(r)
            return acc
          }, {})
          return Object.entries(grouped).map(([rKind, items]) => {
            const cols = RESOURCE_COLUMNS[rKind] ?? [{ key: "name", label: "Name" }, { key: "age", label: "Age" }]
            return (
              <section key={rKind} className="rounded-lg border bg-card shadow-sm p-4 col-span-full">
                <h2 className="text-sm font-semibold mb-3">{RESOURCE_LABELS[rKind] ?? rKind}</h2>
                    ...
                    <tbody>
                      {items.map((item) => (
                        <tr key={item.name} className="border-b hover:bg-muted/50 cursor-pointer"
                          onClick={() => onNavigate(`/${enc(context)}/${enc(namespace)}/${enc(rKind)}/${enc(item.name)}`)}>
                          {cols.map((col) => (
