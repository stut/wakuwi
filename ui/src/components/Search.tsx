...
function loadKinds(): string[] {
  try {
    const v: unknown = JSON.parse(localStorage.getItem(LS_KEY) ?? "null")
...
export function Search({
  ...
  const navigate = (r: SearchResult) => {
    onNavigate(
      `/${encodeURIComponent(context)}/${encodeURIComponent(r.namespace)}/${encodeURIComponent(r.kind)}/${encodeURIComponent(r.name)}`,
    )
  }
                  ...
                  {namespaces
                    ...
                    .map((ns) => (
                      <button
                        ...
                        onClick={() =>
                          onNavigate(
                            `/${encodeURIComponent(context)}/${encodeURIComponent(ns)}`,
                          )
                        }
